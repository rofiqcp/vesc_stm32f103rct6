...
int main(void) {
    ...
    /* Simpan SEMUA bit untuk diagnostik, tetapi gate drive hanya memakai bit
     * hard-critical. Run25 menambahkan audit register yang sangat berguna,
     * namun satu mismatch audit-only tidak boleh melatch MOTOR_NOT_READY
     * selamanya ketika DMA current, FOC ISR, dan kalibrasi aktual sehat. */
    g_vesc_sampling_contract_flags = motor_hw_sampling_contract_flags();
    const uint32_t drive_contract_flags =
        g_vesc_sampling_contract_flags & HW_SAMPLING_CONTRACT_DRIVE_CRITICAL_MASK;
    if (drive_contract_flags != 0U) {
        /* Kegagalan hard-critical tetap safety-critical. Management link tetap
         * hidup agar COMM_FW_VERSION/diagnostik dan recalibration tersedia. */
