...
static void reply_fw_version(void) {
    ...
    // Variabel hw: nilai kerja yang menyimpan state atau hasil antara sesuai konteks algoritma pada lingkup ini.
...
static void reply_current_cal(void) {
    ...
    put_u32(p, &i, ADC3->CR1);
    put_u32(p, &i, ADC3->CR2);
    put_u32(p, &i, ADC3->SQR1);
    put_u32(p, &i, ADC3->SQR3);
    put_u32(p, &i, DMA2_Channel5->CCR);
    put_u32(p, &i, DMA2_Channel5->CNDTR);
...
static void process_payload_for_motor(const uint8_t *data, uint16_t len, motor_id_t id) {
    ...
    switch (cmd) {
        ...
        case COMM_GET_DECODED_ADC:
             ...
             * samples. Sesuai urutan reference, PA2 berasal dari ADC2 rank-4
             * dan PA3 dari ADC2 rank-5; keduanya dilatch pada HT berikutnya.
             * Sebelum frame lengkap pertama tersedia jangan membuat reply nol palsu. */
            if (!app_adc_data_ready())
                break;
            // Variabel p: nilai kerja yang menyimpan state atau hasil antara sesuai konteks algoritma pada lingkup ini.
            uint8_t p[17];
            // Variabel bi: nilai kerja yang menyimpan state atau hasil antara sesuai konteks algoritma pada lingkup ini.
...
static void vesc_comm_reply_diag(void) {
    ...
    /* Revision 17 menambahkan breadcrumb penerimaan/penolakan command motor
     * standar tanpa mengubah payload command VESC itu sendiri. */
    p[i++] = 17U;
