...
// Variabel s_sum_ldc: akumulator penjumlahan untuk averaging atau statistik.
// Variabel s_sum_lu: akumulator penjumlahan untuk averaging atau statistik.
// Variabel s_sum_lv: akumulator penjumlahan untuk averaging atau statistik.
static uint64_t s_sum_lu = 0U, s_sum_lv = 0U, s_sum_ldc = 0U;
// Variabel s_sum_rdc: akumulator penjumlahan untuk averaging atau statistik.
// Variabel s_sum_ru: akumulator penjumlahan untuk averaging atau statistik.
// Variabel s_sum_rv: akumulator penjumlahan untuk averaging atau statistik.
static uint64_t s_sum_ru = 0U, s_sum_rv = 0U, s_sum_rdc = 0U;
// Variabel s_sq_ldc: state internal modul yang dipertahankan antar pemanggilan fungsi.
// Variabel s_sq_lu: state internal modul yang dipertahankan antar pemanggilan fungsi.
// Variabel s_sq_lv: state internal modul yang dipertahankan antar pemanggilan fungsi.
static uint64_t s_sq_lu = 0U, s_sq_lv = 0U, s_sq_ldc = 0U;
// Variabel s_sq_rdc: state internal modul yang dipertahankan antar pemanggilan fungsi.
// Variabel s_sq_ru: state internal modul yang dipertahankan antar pemanggilan fungsi.
// Variabel s_sq_rv: state internal modul yang dipertahankan antar pemanggilan fungsi.
...
// Fungsi offset_variance_num: menjalankan operasi offset variance num sesuai tanggung jawab modul dengan input
// tervalidasi dan state yang konsisten.
static uint64_t offset_variance_num(uint64_t sum, uint64_t sum_sq, uint32_t n) {
    if (n == 0U)
        return UINT64_MAX;
    // Variabel nn: nilai kerja yang menyimpan state atau hasil antara sesuai konteks algoritma pada lingkup ini.
...
// Parameter sum_sq: akumulator penjumlahan untuk averaging atau statistik.
// Parameter n: indeks iterasi lokal untuk menelusuri elemen atau sampel secara deterministik.
// Parameter sigma: nilai kerja yang menyimpan state, parameter, atau hasil antara sesuai konteks algoritma pada
// lingkup ini.
// Fungsi variance_below_sigma: menjalankan operasi variance below sigma sesuai tanggung jawab modul dengan
...
static bool variance_below_sigma(uint64_t sum, uint64_t sum_sq, uint32_t n, uint32_t sigma) {
    ...
    // Variabel nn: nilai kerja yang menyimpan state atau hasil antara sesuai konteks algoritma pada lingkup ini.
    uint64_t nn = (uint64_t)n;
    // Variabel lhs: nilai kerja yang menyimpan state atau hasil antara sesuai konteks algoritma pada lingkup ini.
    uint64_t lhs = offset_variance_num(sum, sum_sq, n);
    // Variabel rhs: nilai kerja yang menyimpan state atau hasil antara sesuai konteks algoritma pada lingkup ini.
...
// Parameter sum_sq: akumulator penjumlahan untuk averaging atau statistik.
// Parameter n: indeks iterasi lokal untuk menelusuri elemen atau sampel secara deterministik.
// Fungsi variance_x100: menjalankan operasi variance x100 sesuai tanggung jawab modul dengan input tervalidasi
...
static void validate_cal_channel(uint8_t idx, int32_t mean, uint16_t mn,
    ...
    // Variabel spread: nilai kerja yang menyimpan state atau hasil antara sesuai konteks algoritma pada lingkup ini.
    uint32_t spread = (uint32_t)(mx - mn);

    /* Offset final berasal dari zero-vector PWM aktif dan pada board pengguna
       berada sekitar 1.9k..2.1k count. Hard reject tetap hanya rail 12-bit agar
       toleransi produksi tidak membuat kalibrasi valid gagal palsu. */
    if (mean <= 8 || mean >= 4087) {
        s_cal_fail_range_mask |= bit;
    }

    /* Noise dinilai pada satu kondisi common-mode yang sama. Midpoint min/max
     * dipakai sebagai offset, sedangkan sum/sum_sq hanya untuk statistik noise. */
...
static void cal_accumulate_all(uint16_t l_u, uint16_t l_v, uint16_t l_dc,
    ...
    s_sum_rv += r_v;
    s_sum_rdc += r_dc;
    s_sq_lu += (uint64_t)l_u * l_u;
    s_sq_lv += (uint64_t)l_v * l_v;
    s_sq_ldc += (uint64_t)l_dc * l_dc;
    s_sq_ru += (uint64_t)r_u * r_u;
    s_sq_rv += (uint64_t)r_v * r_v;
    s_sq_rdc += (uint64_t)r_dc * r_dc;
...
static bool calibration_process_isr(uint16_t l_u, uint16_t l_v, uint16_t l_dc,
    ...
    case FOC_CAL_STAGE_LEFT_DRIVEN:
    {
        ...
        if (s_cal_count >= ADC_NEUTRAL_CAL_SAMPLES) {
            ...
            const uint64_t sums[6] = {s_sum_lu, s_sum_lv, s_sum_ldc, s_sum_ru, s_sum_rv, s_sum_rdc};
            const uint64_t sums_sq[6] = {s_sq_lu, s_sq_lv, s_sq_ldc, s_sq_ru, s_sq_rv, s_sq_rdc};
            ...
            for (uint8_t i = 0U; i < 6U; i++) {
                ...
                cal_store_diag(i, midpoint[i], mins[i], maxs[i],
                               sums[i], sums_sq[i], ADC_NEUTRAL_CAL_SAMPLES);
            }
...
static inline void foc_isr_finish_timing(uint32_t start_cycle) {
    ...
    /* Ambang 85% hanya peringatan dini. Overrun baru terjadi bila satu frame
       benar-benar melewati slot 16-kHz. Kebijakan ini juga berlaku pada jalur
       kalibrasi/fault sehingga diagnostik tidak mengabaikan early-return. */
...
// Parameter adc_words: enam word DMA dual-ADC yang memuat kanal arus sinkron kedua motor.
...
void mcpwm_foc_adc_words_isr_timed(const volatile uint32_t adc_words[6], uint32_t start_cycle) {
    ...
    // Variabel vbus_dma_cndtr: tegangan DC bus yang digunakan untuk normalisasi modulasi dan proteksi.
    uint16_t vbus_dma_cndtr = 0U;
    // Variabel vbus_transfer_seen: tegangan DC bus yang digunakan untuk normalisasi modulasi dan proteksi.
    bool vbus_transfer_seen = false;
    // Variabel vraw: nilai kerja yang menyimpan state atau hasil antara sesuai konteks algoritma pada lingkup ini.
    uint16_t vraw = motor_hw_vbus_raw_from_isr(&vbus_dma_cndtr, &vbus_transfer_seen);
    /* Freshness proof tidak boleh hanya melihat CNDTR. Circular DMA dapat macet
       selamanya pada nilai legal 1/2 sehingga tegangan bus lama tampak valid.
       Flag HTIF5/TCIF5 yang dilatch hardware membuktikan transfer benar-benar
       terjadi sejak ISR FOC sebelumnya, tanpa menambah IRQ ADC3 16 kHz. */
    // Variabel dma_alive: nilai kerja yang menyimpan state atau hasil antara sesuai konteks algoritma pada lingkup ini.
    const bool dma_alive = ((DMA2_Channel5->CCR & DMA_CCR_EN) != 0U);
    // Variabel fresh: nilai kerja yang menyimpan state atau hasil antara sesuai konteks algoritma pada lingkup ini.
    const bool fresh = dma_alive && (vbus_dma_cndtr == 1U || vbus_dma_cndtr == 2U) && vbus_transfer_seen;
    if (!fresh) {
        if (s_vbus_dma_stale_count < UINT8_MAX)
            s_vbus_dma_stale_count++;
