...
extern volatile uint32_t g_adc_dual_dma[15];

/* ADC rank 1..5 mengikuti firmware hoverboard V15 persis. DMA menampung
 * tiga frame berturut-turut; TC muncul setelah frame ke-3. ISR memakai frame
 * terbaru saja, sedangkan dua frame sebelumnya hanya ditulis DMA tanpa kerja
 * CPU. ADC3/DMA2 tetap tidak digunakan. */
...
enum {
    ...
    HW_SAMPLING_CONTRACT_ADC_CLOCK = 1UL << 16,
    HW_SAMPLING_CONTRACT_ADC_SAMPLE_TIMES = 1UL << 17,
    HW_SAMPLING_CONTRACT_DMA1_IRQ_MODE = 1UL << 18,
    HW_SAMPLING_CONTRACT_ADC3_SAMPLE_TIME = 1UL << 19,
    HW_SAMPLING_CONTRACT_TRIGGER_ROUTE = 1UL << 20
};

/* Bit 0..15 membuktikan struktur timer/ADC/DMA. ADC clock dan sumber IRQ TC
 * juga hard-critical. Pemeriksaan sample-time dan AFIO/EXTSEL tambahan tetap
 * audit-only seperti Run31. */
#define HW_SAMPLING_CONTRACT_DRIVE_CRITICAL_MASK \
    (0x00001FFFUL | HW_SAMPLING_CONTRACT_ADC_CLOCK | HW_SAMPLING_CONTRACT_DMA1_IRQ_MODE)
...
// Fungsi motor_hw_sampling_drive_flags: membaca hanya bit kontrak sampling yang hard-critical untuk izin drive.
...
// Fungsi motor_hw_sampling_drive_valid: memastikan kontrak hard-critical drive valid tanpa menjadikan audit-only sebagai latch permanen.
...
// Fungsi hw_status_notify_calibration_start: mengantrikan beep pendek ketika kalibrasi arus mulai tanpa
// memblokir ISR, komunikasi, atau thread motor.
void hw_status_notify_calibration_start(void);
// Parameter ok: true jika kalibrasi berhasil dan offset boleh dipakai oleh FOC.
// Fungsi hw_status_notify_calibration_done: mengantrikan beep hasil kalibrasi; kegagalan diserahkan ke pola fault.
void hw_status_notify_calibration_done(bool ok);
// Fungsi hw_status_notify_eeprom_saved: mengantrikan lima beep pendek untuk setiap transaksi EEPROM/flash yang tersimpan sukses.
