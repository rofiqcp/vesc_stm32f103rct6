...
// Parameter t: jenis kendali ADC yang akan diperiksa apakah titik netralnya berada di voltage_center.
// Fungsi control_is_centered: menentukan mode analog center yang dapat bekerja tanpa tombol eksternal.
static bool control_is_centered(adc_control_type t) {
    return t == ADC_CTRL_TYPE_CURRENT_REV_CENTER ||
           t == ADC_CTRL_TYPE_CURRENT_NOREV_BRAKE_CENTER ||
           t == ADC_CTRL_TYPE_DUTY_REV_CENTER ||
           t == ADC_CTRL_TYPE_PID_REV_CENTER;
}

// Parameter t: jenis kendali ADC yang akan diperiksa dukungan hardware-nya.
// Fungsi control_is_supported_analog: menerima seluruh mode VESC yang hanya membutuhkan PA2/PA3.
static bool control_is_supported_analog(adc_control_type t) {
    return t == ADC_CTRL_TYPE_NONE ||
           t == ADC_CTRL_TYPE_CURRENT ||
           t == ADC_CTRL_TYPE_CURRENT_REV_CENTER ||
           t == ADC_CTRL_TYPE_CURRENT_NOREV_BRAKE_CENTER ||
           t == ADC_CTRL_TYPE_CURRENT_NOREV_BRAKE_ADC ||
           t == ADC_CTRL_TYPE_DUTY ||
           t == ADC_CTRL_TYPE_DUTY_REV_CENTER ||
           t == ADC_CTRL_TYPE_PID ||
           t == ADC_CTRL_TYPE_PID_REV_CENTER;
}
...
static void apply_to_motor(MotorRuntime *m, adc_control_type ctrl, float cmd) {
    ...
    else if (ctrl == ADC_CTRL_TYPE_CURRENT_REV_CENTER) {
        motor_set_current_rel(m, clampf_local(cmd, -1.0f, 1.0f));
    }
    ...
    else if (ctrl == ADC_CTRL_TYPE_DUTY_REV_CENTER) {
        motor_set_duty(m, clampf_local(cmd, -1.0f, 1.0f) * fabsf(m->max_duty));
    }
    else if (ctrl == ADC_CTRL_TYPE_PID) {
        motor_set_speed(m, clampf_local(cmd, 0.0f, 1.0f) * fabsf(m->max_erpm));
    }
    else if (ctrl == ADC_CTRL_TYPE_PID_REV_CENTER) {
        const float rel = clampf_local(cmd, -1.0f, 1.0f);
        const float erpm = rel >= 0.0f ? rel * fabsf(m->max_erpm) : (-rel) * m->min_erpm;
        motor_set_speed(m, erpm);
    }
}
...
void app_adc_service_1khz(uint32_t now_ms) {
    ...
    float pwr;
    if (control_is_centered(c->ctrl_type)) {
        /* Sama dengan VESC: voltage_center dipetakan ke 0.5 dahulu, lalu
         * setelah inversion dikonversi menjadi rentang -1..+1. */
        if (v1 < c->voltage_center) {
            pwr = 0.5f * map01(v1, c->voltage_start, c->voltage_center);
        }
        else {
            pwr = 0.5f + 0.5f * map01(v1, c->voltage_center, c->voltage_end);
        }
    }
    else {
        pwr = map01(v1, c->voltage_start, c->voltage_end);
    }
    ...
    const bool neutral = fabsf(s_adc.ramp) <= 0.01f && fabsf(pwr) <= 0.01f &&
                         (!control_uses_brake_adc(c->ctrl_type) || brake <= 0.01f);
    // Variabel config_mode: data konfigurasi yang mengatur perilaku firmware.
    const bool config_mode = control_is_supported_analog(c->ctrl_type);
    if (!config_mode)
