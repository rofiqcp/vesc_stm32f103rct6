...
status=read('src/hwconf/hw_status.c'); statush=read('src/hwconf/hw.h'); tasks=read('src/motor/mc_interface_tasks.c')
...
ok('motor_hw_emergency_all_off()' in cmd and 'hw_status_power_hold(false)' in cmd,
...
ok('hw_status_power_hold(bool on)' in status and 'OFF_PORT, OFF_PIN' in status, 'PA5 power-hold has a single explicit StatusIO backend')
ok('hw_status_tone_start' in status and 'hw_status_tim3_irq_handler' in status,
   'buzzer tones are generated by the non-blocking timer IRQ backend')
ok('s_tone_toggle_remaining' not in status,
   'unused finite-toggle counter is removed; the status state machine owns duration')
...
ok('MOTOR_FAULT_FLASH_CONFIG' in status, 'flash integrity fault participates in canonical LED+buzzer fault reporting')
...
ok('conf_general_integrity_ok()' in cmd and 'hw_status_power_is_held()' in cmd, 'communications diagnostics expose integrity and power-latch status')
