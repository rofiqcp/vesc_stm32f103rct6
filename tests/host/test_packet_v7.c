...
int main(void) {
    ...
    /* Upstream rejects a 16-bit length prefix for payloads shorter than 255. */
    uint8_t noncanonical[]={3,0,1,0,0,0,3};
    vesc_packet_parser_init(&s); got=0; feed(&s,noncanonical,sizeof(noncanonical));
    assert(got==0);
    feed(&s,out,n);
    assert(got==1 && got_payload[0]==0);
